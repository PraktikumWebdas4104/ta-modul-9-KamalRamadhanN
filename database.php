...
class database{
	...
	function input($nama,$alamat,$usia,$genre,$wisata){
		$ckb = implode(", ",$genre);
		$ws = implode(", ",$wisata);
		$que = mysqli_query($this->conn,"INSERT INTO data(nama, alamat, usia, id, genre, wisata) VALUES ('$nama','$alamat','$usia','','$ckb','$ws')");//buatlah method input
	...
	function update($id,$nama,$alamat,$usia,$genre,$wisata){
		$ckb = implode(", ",$genre);
		$ws = implode(", ",$wisata);
		$que = mysqli_query($this->conn,"UPDATE data SET nama='$nama',alamat='$alamat',usia='$usia',genre='$ckb',wisata='$ws' WHERE id='$id'");//buatlah method update
